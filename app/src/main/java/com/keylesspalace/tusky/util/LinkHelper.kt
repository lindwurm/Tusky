 ...
 */
fun setClickableText(view: TextView, content: Spanned, mentions: Array<Status.Mention>?, listener: LinkListener, removeQuote: Boolean) {
    ...
    for (span in urlSpans) {
        ...
        replaceSpan(builder, span, getLinkSpan(span.url, text.toString(), listener), removeQuote)
    }
    ...
    for (span in otherSpans) {
        ...
        replaceSpan(builder, span, customSpan, false)
    }
 ...
 */
private fun replaceSpan(builder: SpannableStringBuilder, oldSpan: ParcelableSpan?, newSpan: ClickableSpan?, removeQuote: Boolean) {
    ...
    if (start >= 13 && end < builder.length && removeQuote) {
        if (builder.subSequence(start - 13, start).toString() == "\n~~~~~~~~~~\n["
                && builder[end] == ']') {
...
private fun getLinkSpan(url: String, text: String, listener: LinkListener): ClickableSpan {
    return object: CustomURLSpan(url) {
        override fun onClick(widget: View?) {
            listener.onViewUrl(url, text)
        }
    }
}
